...
const toIcsCode = (rawCourseNumber: string): string => {
...
const parseRows = (csvText: string): ParsedRow[] => {
  ...
  return lines.slice(1).map((line) => {
    ...
    const courseCode = toIcsCode(cols[courseNumberIndex] ?? '');
...
const importIcsCourses = async () => {
  const importPath = process.argv[2] || 'data/ics-courses.csv';
  ...
  console.log(`Imported ${courses.size} ICS courses from ${absolutePath}`);
};
